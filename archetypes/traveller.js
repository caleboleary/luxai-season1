...
//traveller = fills cargo, looks for uninhabited resources, avoids cities on path to keep personal fuel
const traveller = (unit, gameState) => {
  unit.name = "traveller";
  const player = gameState.players[gameState.id];

  if (unit.isWorker() && unit.canAct()) {
    if (unit.getCargoSpaceLeft() > 0 && !unit.travelStarted) {
      //gather at least a night's worth of resources
      goToNearestMineableResource(unit, gameState);
    } else {
      //look for uninhabited resources
      unit.travelStarted = true;
